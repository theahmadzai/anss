...
@section('content')
    ...
    <section class="section">
        ...
        <a class="button" href="{{ url('profile') }}">View profile</a>

        <form class="form" method="POST" action="{{ url('profile/'. Auth::user()->id) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')

            <img src="{{ Auth::user()->avatar }}" alt="{{ Auth::user()->name }}" style="width:200px; height:200px; margin-top:1rem;">
            ...
            <div class="form__item">
                ...
                <input class="input" type="date" name="dob" value="{{ old('dob', Auth::user()->dob->format('Y-m-d')) }}">
                @if ($errors->has('dob'))
